...
folders = ["resultado2/ReA", "resultado2/AbsA", "resultado2/2DReA", "resultado2/3DAbsA", "resultado2/3DReA"]
...
for tau in taus:
    ...
    D0 = 0.08
    r = 7.0
    A = (p * (np.sqrt(X**2 + Y**2) - r + 1j * Z)).astype(np.complex64)
    ...
    plt.tight_layout()
    plt.savefig(f"resultado2/ReA/ReA_tau_{tau:.2f}.png")
    plt.close()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig(f"resultado2/AbsA/AbsA_tau_{tau:.2f}.png")
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f"resultado2/2DReA/2DReA_tau_{tau:.2f}.png")
    plt.close()
    ...
    #    for j in range(Ny):
    #        for k in range(Nz):
    #            if values[i][j][k] <= -0.5 or values[i][j][k] >= 0.5:
    #              values[i][j][k] = np.nan
    #scatter = ax.scatter(X, Y, Z, c=values, cmap='RdBu', linewidth=0, antialiased=True)
    ax.set_title(f"Re(A), tau={tau:.2f}")
    fig.colorbar(scatter, ax=ax)
    plt.tight_layout()
    plt.savefig(f"resultado2/3DReA/3DReA_tau_{tau:.2f}.png")
    plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("resultado2/maxAbsA_vs_tau.png")
plt.show()
